...
public sealed partial class FelinidSystem : EntitySystem
{
    ...
    {
        base.Initialize();
        SubscribeLocalEvent<FelinidAbilityComponent, ComponentInit>(OnInit);
        SubscribeLocalEvent<FelinidAbilityComponent, HairballActionEvent>(OnHairball);
        SubscribeLocalEvent<FelinidAbilityComponent, EatMouseActionEvent>(OnEatMouse);
        SubscribeLocalEvent<FelinidAbilityComponent, DidEquipHandEvent>(OnEquipped);
        SubscribeLocalEvent<FelinidAbilityComponent, DidUnequipHandEvent>(OnUnequipped);
    ...
    public override void Update(float frameTime)
    {
        ...
        foreach (var (hairballComp, catComp) in EntityQuery<CoughingUpHairballComponent, FelinidAbilityComponent>())
        {
    ...
    private void OnInit(EntityUid uid, FelinidAbilityComponent component, ComponentInit args)
    {
    ...
    private void OnEquipped(EntityUid uid, FelinidAbilityComponent component, DidEquipHandEvent args)
    {
    ...
    private void OnUnequipped(EntityUid uid, FelinidAbilityComponent component, DidUnequipHandEvent args)
    {
    ...
    private void OnHairball(EntityUid uid, FelinidAbilityComponent component, HairballActionEvent args)
    {
    ...
    private void OnEatMouse(EntityUid uid, FelinidAbilityComponent component, EatMouseActionEvent args)
    {
    ...
    private void SpawnHairball(EntityUid uid, FelinidAbilityComponent component)
    {
        var hairball = Spawn(component.HairballPrototype, Transform(uid).Coordinates);
        var hairballComp = Comp<HairballComponent>(hairball);

        if (TryComp<BloodstreamComponent>(uid, out var bloodstream) && bloodstream.MetabolitesSolution.HasValue)
        {
            var temp = _solutionSystem.SplitSolution(bloodstream.MetabolitesSolution.Value, 20);
    ...
    {
        if (HasComp<FelinidAbilityComponent>(args.Target) || !HasComp<StatusEffectsComponent>(args.Target))
            return;
    ...
    {
        if (HasComp<FelinidAbilityComponent>(args.User) || !HasComp<StatusEffectsComponent>(args.User))
            return;
